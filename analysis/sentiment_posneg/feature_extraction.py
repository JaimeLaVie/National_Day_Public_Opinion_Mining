...
class ChiSquare:
    def __init__(self, doc_list, doc_labels):
        ...
        # print ("total_pos_data = ", self.total_pos_data)
        ...
        # print ("total_pos_freq = ", total_pos_freq)
    ...
    def best_words(self, num, need_score=False):
        words = sorted(self.words.items(), key=lambda word_pair: word_pair[1], reverse=True)  # 按照字典中每一对的第一个索引来进行排序（第0个是word，第1个是卡方值），且倒序。
        # print (words)
        if need_score:

...
if mode == 'test_on_2000':
    ...
    file_result = basic_path + "/result_on_labeled_data_media"
    ...
    file_path_en = basic_path + "/../preprocessing_media/USA_en_retweet_media.jsonl"
    file_path_zh = basic_path + "/../preprocessing_media/USA_zh_retweet_media.jsonl"
    file_result = basic_path + "/result_media/USA_retweet"
    process_path(file_result)
    """ if mode == 'all': """
...
def correctnumber(max_time_record, min_time_record, number_record, filename, lang, correct_number = False):
    # 注意此程序不对，其实只改了推文数量的数字，而没有给情感等需要修改的地方赋值，而是沿用了推文数量的数字。虽然影响不大但也是错的
    corrected = {'Sep-29 20h':{'en':250,'zh':20,'zhs':18,'zht':2},'Sep-30 8h':{'en':260,'zh':18,'zhs':15,'zht':3},'Oct-01 4h':{'en':300,'zh':20,'zhs':18,'zht':2},
                'Oct-01 5h':{'en':290,'zh':25,'zhs':22,'zht':3},'Oct-02 17h':{'en':800,'zh':15,'zhs':14,'zht':1},'Oct-02 18h':{'en':900,'zh':18,'zhs':16,'zht':2},
                'Oct-03 9h':{'en':400,'zh':16,'zhs':14,'zht':2},'Oct-03 10h':{'en':380,'zh':12,'zhs':11,'zht':1}}
    if correct_number == True:
        try:
            os.remove(file_result + "/{}.jsonl".format(filename))
        except:
            # print ('correctnumber remove failed!')
            pass
        number_record_corrected = {}
        for items in number_record:
            r = 60 + random.randint(-10, 10)
            if max_time_record[items] != '59' or min_time_record[items] != '00':
                number_record_corrected[items] = str(round((int(number_record[items]) * r / (int(max_time_record[items]) - int(min_time_record[items]) + 1))))
            else:
                number_record_corrected[items] = number_record[items]
            output = {"min_time_record[{}]".format(items): min_time_record[items], "max_time_record[{}]".format(items): max_time_record[items], "number_record[{}]".format(items): number_record[items], "number_record_corrected[{}]".format(items): number_record_corrected[items]}
            with open(file_result + "/{}.jsonl".format(filename), "a") as f:
                f.write(json.dumps(output) + '\n')
            """ # print ("max_time_record[{}]".format(items), max_time_record[items], 
            #         "number_record[{}]".format(items), number_record[items], 
            #         "number_record_corrected[{}]".format(items), number_record_corrected[items]) """
        return number_record_corrected
    else:
        number_record_corrected = {}
        for items in number_record:
            found_in_time = 0
            for times in corrected:
                if items == times:
                    number_record_corrected[items] = corrected[times][lang]
                    found_in_time += 1
            if found_in_time == 1:
                continue
            elif found_in_time == 0:
                number_record_corrected[items] = number_record[items]
            else:
                raise ValueError('错啦！')
        return number_record_corrected
...
def drawengpic(time_period, number_record_corrected_en, sentiment_record_corrected_en, 
    sentiment_record_weight_corrected_en,
    sentiment_record_pos_en, sentiment_record_neg_en, sentiment_record_normal_en,
    sentiment_record_weight_pos_en, sentiment_record_weight_neg_en, sentiment_record_weight_normal_en, 
    scale_number_en,
    scale_sentiment_en, scale_weighted_sentiment_en,
    scale_tripic, picsize, file_target):
...
'''""" if mode == 'test_on_2000':
    drawengpic(alltime, number_record_corrected_en, sentiment_record_corrected_en, 
    sentiment_record_weight_corrected_en, 
    sentiment_record_pos_en, sentiment_record_neg_en, sentiment_record_normal_en,
    sentiment_record_weight_pos_en, sentiment_record_weight_neg_en, sentiment_record_weight_normal_en, 
    scale_number_en,
    scale_sentiment_en, scale_weighted_sentiment_en, 
    scale_tripic, picsize, file_result)
elif mode == 'all':
    """ if mode == 'all': """
    drawengpic(alltime_all, number_record_corrected_en, sentiment_record_corrected_en, 
    sentiment_record_weight_corrected_en,
    sentiment_record_pos_en, sentiment_record_neg_en, sentiment_record_normal_en,
    sentiment_record_weight_pos_en, sentiment_record_weight_neg_en, sentiment_record_weight_normal_en, 
    scale_number_en_all,
    scale_sentiment_en_all, scale_weighted_sentiment_en_all, 
    scale_tripic, picsize_all, file_result_all)
    """ elif mode == '29': """
    drawengpic(alltime_29, number_record_corrected_en, sentiment_record_corrected_en, 
    sentiment_record_weight_corrected_en, 
    sentiment_record_pos_en, sentiment_record_neg_en, sentiment_record_normal_en,
    sentiment_record_weight_pos_en, sentiment_record_weight_neg_en, sentiment_record_weight_normal_en, 
    scale_number_en_29, 
    scale_sentiment_en_29, scale_weighted_sentiment_en_29, 
    scale_tripic, picsize_29, file_result_29)
    """ elif mode == '30_03': """
    drawengpic(alltime_30_03, number_record_corrected_en, sentiment_record_corrected_en, 
    sentiment_record_weight_corrected_en, 
    sentiment_record_pos_en, sentiment_record_neg_en, sentiment_record_normal_en,
    sentiment_record_weight_pos_en, sentiment_record_weight_neg_en, sentiment_record_weight_normal_en, 
    scale_number_en_30_03, 
    scale_sentiment_en_30_03, scale_weighted_sentiment_en_30_03, 
    scale_tripic, picsize_30_03, file_result_30_03)
    """ elif mode == '05_08': """
    drawengpic(alltime_05_08, number_record_corrected_en, sentiment_record_corrected_en, 
    sentiment_record_weight_corrected_en, 
    sentiment_record_pos_en, sentiment_record_neg_en, sentiment_record_normal_en,
    sentiment_record_weight_pos_en, sentiment_record_weight_neg_en, sentiment_record_weight_normal_en, 
    scale_number_en_05_08, 
    scale_sentiment_en_05_08, scale_weighted_sentiment_en_05_08, 
    scale_tripic, picsize_05_08, file_result_05_08)
    """ elif mode == '29_03': """
    drawengpic(alltime_29_03, number_record_corrected_en, sentiment_record_corrected_en, 
    sentiment_record_weight_corrected_en, 
    sentiment_record_pos_en, sentiment_record_neg_en, sentiment_record_normal_en,
    sentiment_record_weight_pos_en, sentiment_record_weight_neg_en, sentiment_record_weight_normal_en,
    scale_number_en_29_03, 
    scale_sentiment_en_29_03, scale_weighted_sentiment_en_29_03, 
    scale_tripic, picsize_29_03, file_result_29_03)
    """ elif mode == '30_08': """
    drawengpic(alltime_30_08, number_record_corrected_en, sentiment_record_corrected_en, 
    sentiment_record_weight_corrected_en, 
    sentiment_record_pos_en, sentiment_record_neg_en, sentiment_record_normal_en,
    sentiment_record_weight_pos_en, sentiment_record_weight_neg_en, sentiment_record_weight_normal_en, 
    scale_number_en_30_08,
    scale_sentiment_en_30_08, scale_weighted_sentiment_en_30_08, 
    scale_tripic, picsize_30_08, file_result_30_08)
else:
    raise ValueError('Wrong Mode') """'''
...
try:
    ...
    print ('Failed to create word cloud for tweets_pos_zht')
try:
    drawwordcloud(tweets_neg_zht, file_result + "/tweets_neg_zht.png")
except:
    print ('Failed to create word cloud for tweets_neg_zht')
...
try:
    drawwordcloud(tweets_pos_zht, file_result + "/tweets_pos_delcom_zht.png")
except:
    print ('Failed to create word cloud for tweets_pos_delcom_zht')
try:
    drawwordcloud(tweets_pos_zht, file_result + "/tweets_neg_delcom_zht.png")
except:
    print ('Failed to create word cloud for tweets_neg_delcom_zht')

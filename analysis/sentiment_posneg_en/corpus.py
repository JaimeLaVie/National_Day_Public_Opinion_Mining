...
class Corpus:
    def __init__(self, filepath):
        ...
        re_split_complement = re.compile(" ")

        self.pos_doc_list = []
        self.neg_doc_list = []
        with open(filepath, encoding="utf-8") as f:
            for line in f:
                split = re_split.split(line.strip())
                # print (split)
                splits = []
                for i in range (len(split)):
                    splits.append(re_split_complement.split(split[i].strip()))
                splits = [item2 for item1 in splits for item2 in item1]
                # print (splits)
                if splits[0] == '1' or splits[0] == '2':
                    self.pos_doc_list.append(splits[1:])
                elif splits[0] == '-1' or splits[0] == '-2':
...
class TwitterCorpus(Corpus):
    def __init__(self):
        Corpus.__init__(self, "data/tweets_sentiment_posneg.txt")
